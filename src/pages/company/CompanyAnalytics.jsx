...
import { 
  ...
  ProgressBar, Tabs, Tab, Alert, Spinner,
  Dropdown, Form, Table, Modal, ListGroup
} from 'react-bootstrap';
import { useNavigate } from 'react-router-dom';
import { 
  FaChartLine, FaChartBar, FaChartPie, FaUsers, 
  FaBriefcase, FaCalendarCheck, FaDollarSign, 
  FaArrowUp, FaArrowDown, FaClock, FaFilter,
  FaDownload, FaSync, FaEye, FaCalendarAlt,
  FaUserCheck, FaUserClock, FaMapMarkerAlt,
  FaIndustry, FaGraduationCap, FaFileExport,
  FaInfoCircle, FaExclamationTriangle, FaCheckCircle,
  FaTimesCircle, FaRegClock, FaChartArea,
  FaDatabase, FaCog, FaRobot, FaBrain,
  FaCloudUploadAlt, FaShieldAlt, FaBell,
  FaTrophy, FaAward, FaMedal, FaCrown,
  FaGem, FaStar, FaHandshake, FaHeart,
  FaComments, FaVideo, FaPhone, FaEnvelope,
  FaShare, FaLink, FaExternalLinkAlt,
  FaSearch, FaPlus, FaEdit, FaTrash,
  FaSave, FaTimes, FaBolt, FaRocket,
  FaLightbulb, FaMagic, FaPalette
} from 'react-icons/fa';
import {
  Line, Bar, Doughnut, Radar, Pie
...
const CompanyAnalytics = () => {
  ...
  const prepareChartData = (data) => {
    ...
    const sourceLabels = ['CareerConnect', 'LinkedIn', 'Indeed', 'Company Website', 'Referrals', 'Other'];
  ...
  const generateAIRecommendations = (data) => {
    ...
    if (data.interviewRate < 30) {
      recommendations.push({
        ...
        description: 'Only ' + data.interviewRate + '% of applicants reach interview stage. Consider refining your screening process.',
        priority: 'high',
        icon: <FaUserCheck />,
    ...
    if (data.hireRate < 15) {
      recommendations.push({
        ...
        description: 'Your hire rate is ' + data.hireRate + '%. Use AI matching to find better candidates.',
        priority: 'high',
        icon: <FaRobot />,
        action: () => navigate('/company/ai-matching')
      });
    ...
    if (data.avgTimeToHire > 30) {
      recommendations.push({
        ...
        description: 'Average ' + data.avgTimeToHire + ' days to hire. Streamline your hiring process.',
        priority: 'medium',
        icon: <FaClock />,
    ...
    if (data.diversityScore < 50) {
      recommendations.push({
        ...
        description: 'Diversity score is ' + data.diversityScore + '%. Consider outreach to diverse candidates.',
        priority: 'medium',
        icon: <FaUsers />,
      ...
      <Container fluid className="company-analytics loading">
        <div className="loading-spinner d-flex flex-column align-items-center justify-content-center min-vh-50">
          ...
          <small className="text-muted">
            Preparing your company performance insights
          </small>
        </div>
      </Container>
    );
  ...
  return (
    <Container fluid className={`company-analytics ${isMobile ? 'mobile-view' : ''} px-md-4 py-md-3 px-2 py-2`}>
      {/* Header */}
              ...
              <div className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                <div className="flex-grow-1">
                  ...
                  <p className="text-muted small mb-0">
                    Gain insights into your hiring performance and optimize your recruitment strategy
                  </p>
                </div>
            ...
            <Card.Body className="p-3 p-md-4">
              <div className="d-flex flex-column flex-md-row justify-content-between align-items-center mb-3 mb-md-4">
                ...
                <div className="performance-score-display">
                  <div className={`score-circle score-${getPerformanceColor(performanceMetrics.overallScore)}`}>
                    <span className="score-value">{performanceMetrics.overallScore}</span>
                    <span className="score-label">/100</span>
                  </div>
                  <div className="ms-3">
                    <div className="small text-muted">Overall Score</div>
                    <div className={`trend-indicator ${performanceMetrics.overallScore > 70 ? 'up' : 'down'}`}>
                      {performanceMetrics.overallScore > 70 ? <FaArrowUp /> : <FaArrowDown />}
                      <span className="ms-1">{performanceMetrics.overallScore > 70 ? 'Good' : 'Needs Improvement'}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <Row className="g-2 g-md-3">
                {[
                  { label: 'Candidate Quality', value: performanceMetrics.candidateQuality, icon: <FaUserCheck /> },
                  { label: 'Time Efficiency', value: performanceMetrics.timeEfficiency, icon: <FaClock /> },
                  { label: 'Cost Effectiveness', value: performanceMetrics.costEffectiveness, icon: <FaDollarSign /> },
                  { label: 'Diversity Score', value: performanceMetrics.diversityScore, icon: <FaUsers /> },
                  { label: 'Employer Brand', value: performanceMetrics.employerBrandScore, icon: <FaPalette /> }
                ].map((metric, index) => (
                  <Col key={index} xl={2} lg={4} md={4} sm={6} xs={12}>
                    <Card className={`metric-card border-0 h-100 bg-${getPerformanceColor(metric.value)}-subtle`}>
      ...
      >
                    ...
                    {[
                      ...
                      <Col key={index} md={4} sm={6} xs={12}>
                        <Card className="metric-mini-card border-0 h-100">
                          <Card.Body className="p-2 p-md-3">
                            <div className="d-flex align-items-center justify-content-between mb-2">
                              <div className={`metric-mini-icon bg-${metric.color}-subtle text-${metric.color}`}>
                                {metric.icon}
                              </div>
                              <div className={`small fw-bold ${metric.change.startsWith('+') ? 'text-success' : 'text-danger'}`}>
                                {metric.change}
                              </div>
                            </div>
                            <h3 className="metric-mini-value mb-1">{metric.value}</h3>
                            <p className="metric-mini-label text-muted small mb-0">{metric.label}</p>
                          </Card.Body>
                        </Card>
                      </Col>
                    ))}
              ...
              <Card className="border-0 shadow-sm h-100">
                <Card.Header className="bg-white border-0 py-2 py-md-3">
                  <div className="d-flex justify-content-between align-items-center">
                    ...
                    <Badge bg="info" className="small">AI Generated</Badge>
                  </div>
                </Card.Header>
                <Card.Body>
                  {insights.length > 0 ? (
                ...
                <Card.Body>
                  <ListGroup variant="flush">
                    {conversionFunnel.labels.map((label, index) => {
                      ...
                      const prevValue = index > 0 ? conversionFunnel.datasets[0]?.data[index - 1] || 100 : 100;
                        ...
                        <ListGroup.Item key={index} className="border-0 px-0 py-2 py-md-3">
                          <div className="d-flex justify-content-between align-items-center">
                            ...
                            <div className="text-end">
                              <Badge bg={conversionRate > 50 ? 'success' : conversionRate > 30 ? 'warning' : 'danger'}>
                          ...
                          <ProgressBar 
                            now={value} 
                            max={1000}
                            variant={index === 0 ? 'primary' : index === 1 ? 'info' : index === 2 ? 'warning' : 'success'}
                            className="mt-2"
                            style={{ height: '4px' }}
                          />
                        </ListGroup.Item>
        ...
        <Tab eventKey="ai-recommendations" title={
          <>
              ...
              <Card className="border-0 shadow-sm h-100">
                <Card.Header className="bg-white border-0 py-2 py-md-3">
                  <div className="d-flex justify-content-between align-items-center">
                    ...
                    <Badge bg="info" className="small">Updated in real-time</Badge>
                  </div>
                </Card.Header>
                <Card.Body>
                  {aiRecommendations.length > 0 ? (
                    <div className="recommendations-grid">
                      {aiRecommendations.map((rec) => (
                        <Card key={rec.id} className={`recommendation-card border-0 mb-3 ${rec.priority === 'high' ? 'border-danger' : rec.priority === 'medium' ? 'border-warning' : 'border-info'}`}>
                          <Card.Body className="p-3">
                            <div className="d-flex align-items-start">
                              <div className={`recommendation-icon me-3 bg-${rec.priority === 'high' ? 'danger' : rec.priority === 'medium' ? 'warning' : 'info'}-subtle text-${rec.priority === 'high' ? 'danger' : rec.priority === 'medium' ? 'warning' : 'info'}`}>
                                {rec.icon}
                              </div>
                              <div className="flex-grow-1">
                    ...
                    <div className="text-center py-5">
                      ...
                      <p className="text-muted mb-3">Continue using the platform to receive personalized recommendations</p>
                    </div>
                  )}
                </Card.Body>
              </Card>
                ...
                <Card.Body>
                  {predictions.length > 0 ? (
                    ...
                    <div className="text-center py-4">
                      ...
                      <p className="text-muted small">Predictive insights will appear as you collect more data</p>
                    </div>
                  )}
                </Card.Body>
              </Card>
          ...
          <div className="d-flex align-items-center">
            ...
            <div className="flex-grow-1">
              ...
              <p className="mb-0 small">
                You need more applications ({50 - stats.totalApplications} more) for accurate analytics. 
                Consider posting more jobs or promoting your openings.
              </p>
            </div>
            <Button 
