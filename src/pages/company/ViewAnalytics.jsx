...
import { 
  ...
  Form, Spinner, Alert, Table, Badge, ProgressBar,
  Tabs, Tab, ListGroup
} from 'react-bootstrap';
import { 
  FaChartBar, FaChartLine, FaChartPie, FaUsers, 
  FaBriefcase, FaCalendarAlt, FaArrowUp, FaArrowDown,
  FaFilter, FaDownload, FaEye, FaClock, FaCheckCircle,
  FaMoneyBillWave, FaPercentage, FaCalendar, FaRegChartBar,
  FaUserCheck, FaHistory, FaDollarSign,
  FaRegClock, FaIndustry, FaMapMarkerAlt
} from 'react-icons/fa';
...
const SimpleLineChart = ({ data, height = 300 }) => {
  ...
  useEffect(() => {
    ...
    // Find max value
    const maxValue = Math.max(...data.map(d => Math.max(d.applications || 0, d.interviews || 0, d.hires || 0)));
...
const SimpleBarChart = ({ data, height = 300 }) => {
  ...
  useEffect(() => {
    ...
    data.forEach((item, i) => {
      ...
      ctx.fillRect(x + barWidth / 2, padding.top + chartHeight - hireHeight, barWidth / 2, hireHeight);
  ...
  const getTopJobsData = () => {
    ...
    return analyticsData.jobs
      ...
      .map(job => ({
        ...
        views: job.views || 0,
        conversion: job.applicantsCount > 0 ? 
          Math.round(((job.applicantsCount || 0) / (job.views || 1)) * 100) : 0
      }));
  };
        ...
        <Col>
          <div className="d-flex justify-content-between align-items-center">
            ...
            <div className="d-flex gap-2">
              ...
              <Dropdown>
                <Dropdown.Toggle variant="primary" className="d-flex align-items-center gap-2" disabled={exportLoading}>
                  {exportLoading ? (
                    <>
            ...
            <Card.Body>
              <div className="d-flex justify-content-between align-items-start">
                <div>
                  ...
                  <h2 className="mb-0">
                    {formatNumber(analyticsData?.stats?.activeJobs || 8)}
                  </h2>
                  <div className="d-flex align-items-center mt-2">
                    <FaArrowUp className="text-success me-1" />
            ...
            <Card.Body>
              <div className="d-flex justify-content-between align-items-start">
                <div>
                  ...
                  <h2 className="mb-0">
                    {analyticsData?.stats?.timeMetrics?.avgTimeToHire || 28}
                  </h2>
                  <div className="d-flex align-items-center mt-2">
                      ...
                      return (
                        <div key={stage.stage} className="funnel-stage mb-3">
                          ...
                          <ProgressBar 
                            ...
                            variant={
                              stage.stage === 'Applied' ? 'primary' :
                              stage.stage === 'Reviewed' ? 'info' :
                              stage.stage === 'Interview' ? 'warning' : 'success'
                            }
                            style={{ height: '20px' }}
                          />
                          {index < getConversionFunnelData().length - 1 && (
                            <div className="text-end small text-muted mt-1">
                              {getConversionFunnelData()[index].count > 0 ? 
                                Math.round((getConversionFunnelData()[index + 1].count / getConversionFunnelData()[index].count) * 100) : 0}% 
                          ...
                          <tr key={index}>
                            ...
                            <td>
                              <div className="d-flex align-items-center">
                                ...
                                <ProgressBar 
                                  now={job.conversion} 
                                  variant={job.conversion > 50 ? 'success' : job.conversion > 30 ? 'warning' : 'danger'}
                            ...
                            {[
                              ...
                              <div key={item.level} className="mb-3">
                                ...
                                <ProgressBar 
                                  now={item.percentage} 
                                  variant={
                                    item.percentage > 30 ? 'success' : 
                                    item.percentage > 20 ? 'info' : 
                                    item.percentage > 10 ? 'warning' : 'danger'
                                  }
                                />
                              </div>
                            ))}
              ...
              <Row>
                <Col md={4}>
                  <Alert variant="info" className="h-100">
                    ...
                    <p className="mb-0 small">
                      Average response time is 2.5 days. Aim for 1 day to improve candidate experience.
                    </p>
                  </Alert>
                </Col>
                <Col md={4}>
                ...
                <Col md={4}>
                  <Alert variant="warning" className="h-100">
                    ...
                    <p className="mb-0 small">
                      Interview to hire rate is 25%. Consider improving interview assessment methods.
                    </p>
                  </Alert>
                </Col>
              </Row>
