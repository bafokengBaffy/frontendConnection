...
export const teamService = {
  async getTeamMembers(filters = {}) {
    try {
      ...
      const { role = 'all', status = 'active', sortBy = 'joinedAt', sortOrder = 'desc' } = filters;

      const teamRef = collection(db, COLLECTIONS.COMPANY_TEAM);
      let q = query(teamRef, where('companyId', '==', companyId), orderBy(sortBy, sortOrder));
  ...
  validateTeamMemberData(data) {
    ...
    if (
      !data.role ||
      !['admin', 'recruiter', 'hiring_manager', 'interviewer', 'viewer'].includes(data.role)
    ) {
...
export const documentsService = {
  async getDocuments(filters = {}) {
    try {
      ...
      let q = query(docsRef, where('companyId', '==', companyId), orderBy(sortBy, sortOrder));
...
export const settingsService = {
  async getCompanySettings() {
    try {
      ...
      const q = query(settingsRef, where('companyId', '==', companyId));
