...
export const useProfileUpload = (options = {}) => {
  ...
  const uploadProfilePhoto = useCallback(async (file, retryAttempt = 0) => {
    if (!currentUser?.uid) {
  ...
  }, [currentUser?.uid, validateFile, onUploadStart, onUploadComplete, onUploadError, simulateProgress, resetUpload]);
...
export const useDragDropUpload = (onFilesDrop, options = {}) => {
  ...
  const validateDroppedFiles = useCallback((files) => {
    // Check file count
  ...
  const setupDragDrop = useCallback((element) => {
  ...
  }, [handleDragEnter, handleDragLeave, handleDragOver, handleDrop, handleTouchStart, handleTouchEnd, disabled]);
