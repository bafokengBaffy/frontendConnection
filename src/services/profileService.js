  ...
  async uploadResume(userId, file) {
    try {
      ...
      try {
        ...
        console.warn('⚠️ Cloudinary resume upload failed, falling back to Firebase:', cloudinaryError);
  ...
  async uploadDocument(userId, file, documentType, metadata = {}) {
    try {
      ...
      if (file.type.startsWith('image/')) {
        ...
        uploadResult = await this.uploadToFirebaseStorage(userId, file, `documents/${documentType}`);
      }
